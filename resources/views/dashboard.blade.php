...
function fetchAllProducts() {
    fetch('/api/products')
        ...
        .then(res => {
            if(res.success) {
                console.log("Watches loaded:", res.data);
                // Loop through res.data to render watches dynamically in HTML
            }
        });
}

// Connects UI to POST http://127.0.0.1:8000/api/product
function saveNewProduct() {
    const data = {
        name: document.getElementById('pm_name').value, // Matches modal input id
    ...
    .then(response => response.json())
    .then(res => {
        showToast(res.message, "success"); // Triggers dashboard toast
        closeModal('productModal'); // Closes modal
...
<body>
    ...
    <!-- <script> 
        ...
        // Connects UI to POST http://127.0.0.1:8000/api/product
        function saveNewProduct() {
            // Safety check: Make sure elements exist before reading values
            const nameEl = document.getElementById('pm_name');
            const priceEl = document.getElementById('pm_price');
            const qtyEl = document.getElementById('pm_qty');
            const descEl = document.getElementById('pm_desc');

            if (!nameEl || !priceEl || !qtyEl || !descEl) {
                console.error("Missing one or more form input fields in the HTML!");
                return;
            }
